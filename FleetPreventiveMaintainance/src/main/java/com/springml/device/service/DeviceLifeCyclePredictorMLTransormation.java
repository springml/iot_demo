...
public class DeviceLifeCyclePredictorMLTransormation extends DoFn<TableRow, TableRow> {
    ...
    private TableRow getTableRowFromResponse(String content, String unitNumber) {
        TableRow tableRow = new TableRow();
        Gson gson = new Gson();
        try {
            LifeCyclePredictionResponse predictiosResponse = gson.fromJson(content, LifeCyclePredictionResponse.class);
            tableRow.set("UnitNumber", unitNumber);
            float remainingOperationCycles = predictiosResponse.getPredictions()[0].getOutputs();
    ...
    private Object getAsCloudMLRequest(TableRow sensorDetails) {
        ...
        // map.put("UnitNumber", sensorDetails.get("UnitNumber"));
        map.put("UnitNumber", 100);
        ...
        //  map.put("RemainingUsefulLife",131);
        instances.add(map);
        return instances;
    }
    ...
    protected TableRow getPredictedLifeCycle(TableRow sensorDetails) {
        ...
        try {
            ...
            String predictionResponse = deviceLifeCyclePredictorClient.getPredictedLifeCycle(jsonHttpContent, fleetPreventiveMLUrl);
